...
from libs.weaviate_lib import COLLECTION_CATEGORIES, COLLECTION_FEED_COMMENTS, delete_collection_object, search_non_vector_collection, update_collection_object, insert_to_collection, delete_collection_objects_many
...
def get_all_categories(limit: int, offset: int) -> list[dict]:
    """Get all categories with pagination"""
    categories = search_non_vector_collection(COLLECTION_CATEGORIES, limit=limit, offset=offset, properties=["name", "description", "created_at", "updated_at"])
    return categories
...
def get_category_by_id(category_id: str) -> dict:
    ...
    categories = search_non_vector_collection(COLLECTION_CATEGORIES, filters=filters, properties=["name", "description", "created_at", "updated_at"])
